...
import {
    ...
    Users, Briefcase, TrendingUp, Star, ArrowRight, Receipt, BarChart3, UserIcon, Package, BrainCircuit, ShieldCheck,
    Award, Settings, Users2, BookOpen, Server, Rocket, CheckCircle, CreditCard, Activity,
    BarChartBig, ChevronLeft, ChevronUp, Asterisk, // Using Sparkles as a fallback for Asterisk
...
// Define iconComponents at the module level or ensure it's defined before use if inside the component.
// For simplicity and to avoid potential re-declaration issues in React, defining it outside is often cleaner.
...
export default function Header() {
  ...
  if (!isMounted) {
    ...
    // The green/pink background for the top bar will be provided by AppLayout
    return (
      <header className="w-full fixed top-0 left-0 right-0 z-50">
        <div className={cn(isLandingPage ? 'bg-wz-green' : 'bg-wz-pink', "h-16")}> {/* Height of the top bar area including its padding */}
          {/* Skeleton/Placeholder for top bar content */}
        </div>
        <div className="bg-nav-secondary h-10 sticky top-[64px] z-30 shadow-md"> {/* Assuming top bar area is 64px */}
          {/* Skeleton/Placeholder for second nav bar */}
        </div>
      </header>
    );
  }
            ...
            <div className="flex items-center space-x-2">
              {!isAuthenticated && !isLoadingSession ? (
                <>
                  <Button
                    asChild
                    className="btn-wz btn-wz-pink hover:ring-1 hover:ring-wz-border-dark hover:ring-offset-1 hover:ring-offset-white text-sm !py-1.5 !px-5 whitespace-nowrap shadow-wz-hard-sm"
                  >
  ...
  return (
    <header className="w-full fixed top-0 left-0 right-0 z-40">
      {/* Top Row - Pink for Internal Pages, background provided by AppLayout */}
      <div className="h-12"> {/* Top bar itself is transparent to pick up AppLayout's pink */}
        <div className="container-default flex items-center h-full">
          {/* Left: Logo & Wallet Info */}
          <div className="flex items-center space-x-3">
            <Link href="/" className="flex items-center space-x-2 no-underline">
              {getIcon("Sparkles", { className: "h-7 w-7 text-wz-pink" })} {/* Logo uses Wzuh Pink for Sparkles */}
          ...
          {/* Right: Auth Buttons & Mobile Menu Trigger */}
           <div className="ml-auto flex items-stretch h-full text-sm">
            {!isAuthenticated && !isLoadingSession ? (
              <div className="flex items-stretch h-full">
                ...
                <div className="flex-1 flex items-center justify-center border-l border-wz-pink/30">
                    <Button asChild variant="default" className="btn-wz bg-wz-purple text-wz-pink hover:bg-white hover:text-wz-pink rounded-md px-3 py-1.5 text-sm font-semibold shadow-wz-hard-sm whitespace-nowrap w-full h-full !rounded-none !border-0">
                        <Link href="/get-started">Get Started</Link>
                    </Button>
                </div>
              </div>
                        ...
                        <SheetClose key={`${link.href}-mobile-app`} asChild>
                          <Link
                            ...
                            className={cn(
                              "flex items-center px-3 py-2.5 rounded-md text-base font-medium transition-colors whitespace-nowrap no-underline",
                              isActive
                                ? "bg-white text-wz-pink font-semibold shadow-sm" // Active style
                                : "text-wz-pink hover:bg-white/70 hover:text-pink-700" // Default and hover
                            )}
                          >
      ...
      <div className="bg-nav-secondary shadow-md h-10 sticky top-[48px] z-30"> {/* sticky top value should be height of top bar */}
        <div className="container-default flex h-full items-stretch">
          <nav className="flex items-stretch h-full overflow-x-auto whitespace-nowrap flex-grow scrollbar-hide">
              ...
              return (
                ...
                >
                  <Link
                    href={link.href}
                    className={cn(
                      "flex items-center justify-center w-full h-full px-5 text-sm font-medium no-underline transition-all duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-nav-secondary-active-border focus-visible:ring-offset-1 focus-visible:ring-offset-nav-secondary", // padding on Link for full hover area
                      isActive
                        ? "bg-white text-wz-pink rounded-t-md font-semibold relative border-b-2 border-wz-pink" // Active: white bg, pink text, pink underline
                        : "text-wz-pink hover:bg-white hover:text-wz-pink hover:rounded-t-md" // Default: pink text. Hover: white bg, pink text
                    )}
                  >
                    {link.label}
                  </Link>
           ...
           <div className="hidden md:flex items-center space-x-5 pl-8 pr-4 border-l border-nav-secondary-fg/20">
             {socialMediaLinks.map((sLink) => (
